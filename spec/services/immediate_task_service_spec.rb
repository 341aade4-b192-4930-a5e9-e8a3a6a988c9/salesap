...
RSpec.describe 'ImmediateTaskService' do
  ...
  it {
    ...
    expect(result.first['task_id']).to eq task_3_1.id
    expect(result.second['task_id']).to eq task_3_2.id
    expect(result.third['task_id']).to eq task_3_3.id
    expect(result[4]['task_id']).to be nil
  }
